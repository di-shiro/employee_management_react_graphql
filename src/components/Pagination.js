          ...
          <div>
            <SearchIcon
              ...
              onClick={async () => {
                ...
                setLast(0); // 検索処理を実行後にinput form をリセット
              }}
            />
          </div>
          <ul className={styles.pagination__list}>
        ...
        <Grid item xs={4}>
          ...
          {dataPages.allDepartments.pageInfo.hasNextPage && (
            <button
              onClick={() => {
                /*
                    下記のfetchMoreでサーバから次のページのデータを取得する処理。
                    データ取得後、その内部のupdateQueryで、
                    この<Grid item>内で表示しているDepartment一覧を更新する。
                    ボタン押下前のリストと押下後に取得したリストを結合することで、
                    上の<ul>内のdataPagesに最新データが入ってくる。
                     */
                fetchMore({
                  variables: {
                    first: NUM_PAGE, // 取得する要素数（データ数）
                    // 現在のページの最後の要素のcursorの値（ページの境目となる要素を指定する）
